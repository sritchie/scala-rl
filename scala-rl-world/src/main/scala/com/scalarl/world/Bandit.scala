...
import com.scalarl.state.MapState
...
object Bandit {
  ...
  /** An "Arm" is something that takes you to a new state. We just happen to have only a single
    * state here, so it always takes you back to a given "bandit" problem.
    */
  ...
  /** Returns a Generator that splits out states for each of the games to play. This generator
    * evolves in a non-stationary way.
    *
    * The set below is totally fucked... it's returning a SINGLE generator each time, not the good
    * stuff that we need.
    */
  def nonStationary(
      nArms: Int,
