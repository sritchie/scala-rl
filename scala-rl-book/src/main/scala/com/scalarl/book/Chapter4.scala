/** Dynamic programming chapter. This should have similar examples to what we generated for Chapter
  * 3.
  */
package com.scalarl
package book
...
import com.scalarl.world.{CarRental, GridWorld}
...
object Chapter4 {
  ...
  def shouldStop[Obs, T: ToDouble](
    ...
    if (verbose) {
      ...
      println(
        s"""Max diff seen: ${Util
            .diff[Obs](l.seen ++ r.seen, doubleFn(l), doubleFn(r), _.max(_))}"""
      )
    }
  ...
  def fourOne(
      inPlace: Boolean
  ): (StateValueFn[Position, DecayState[Double]], Long) =
  ...
  /** The big differences from the book version are:
    *
    * \- Currently our Poisson distribution normalizes over the allowed values, rather than just
    * truncating the chance of a value greater than the max to zero. \- our Greedy policy randomly
    * chooses from the entire greedy set, vs just choosing the "first" thing, like Numpy does.
    *
    * The Python version also keeps an actual greedy policy, which means that the policy starts by
    * returning 0 no matter what, by design, instead of by acting as a random policy until it knows
    * any better.
    *
    * Without that the generated values match.
    *
    * TODO ALSO... currently, the sweepUntil function only supports valueIteration or updating on
    * every single sweep. The book actually wants to do a full round of policy evaluation before
    * doing any policy improvement.
    *
  ...
  def fourTwo(
      ...
      StateValueFn[CarRental.InvPair, DecayState[Double]],
      CarRental.Config,
      Long
  ) = {
    ...
    val (roundOne, _) =
      Sweep.sweepUntil[CarRental.InvPair, CarRental.Move, Double, DecayState[
        Double
      ], Cat, Cat](
        empty,
    ...
    println(
      s"""Stable? ${Sweep
          .isPolicyStable[CarRental.InvPair, CarRental.Move, Double, DecayState[
            Double
          ], Cat, Cat](
            empty,
            roundOne,
  ...
  /** This currently is not great because we don't have a way of automatically binning the data and
    * generating that graph. This is custom.
    */
  def vfToSeqPoints(
      vf: StateValueFn[CarRental.InvPair, DecayState[Double]]
  ): Seq[Seq[Double]] =
    (0 to 20).map { row =>
      (0 to 20).map { col =>
        ToDouble[DecayState[Double]].apply(
          vf.stateValue(
            (CarRental.Inventory(row, 20), CarRental.Inventory(col, 20))
          )
        )
      }.toSeq
    }.toSeq
  ...
  /** I'm leaving this in a nightmare state for now. To finish this out, we really need to:
    *
    * \- add support for policy evaluation and policy stability checks, alternating. \- come up with
    * some way of actually turning a particular policy's decisions into a heat map that's not so
    * hardcoded \- NOT have the graph library explode when I cancel a run, for Heatmap.
    */
  def runCarRental(): Unit = {
    ...
    val estimator: ActionValue[CarRental.InvPair, CarRental.Move, Double, DecayState[
      Double
    ], Cat] =
      Evaluator.oneAhead(
        vf,
