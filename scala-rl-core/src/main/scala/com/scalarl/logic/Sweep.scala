...
object Sweep {
  ...
  /** This sweeps across the whole state space and updates the policy every single time IF you set
    ...
    *
    * What we really want is the ability to ping between updates to the value function or learning
    * steps; to insert them every so often.
    *
    * This function does NOT currently return the final policy, since you can just make it yourself,
    * given the return value and the function.
    */
  def sweep[Obs, A, R, T, M[_]: Expectation, S[_]: Expectation](
    ...
    states
      .foldLeft(
        (valueFn, evaluatorFn(valueFn, policyFn(valueFn)), policyFn(valueFn))
      ) { case ((vf, ev, p), state) =>
  ...
  def sweepUntil[Obs, A, R, T, M[_]: Expectation, S[_]: Expectation](
    ...
    Monad[Id].tailRecM((valueFn, 0L)) { case (fn, nIterations) =>
      val updated =
        sweep(fn, policyFn, evaluatorFn, states, inPlace, valueIteration)
      Either.cond(
  ...
  /** Helper to tell if we can stop iterating. The combine function is used to aggregate the
    * differences between the value functions for each observation... the final aggregated value
    * must be less than epsilon to return true, false otherwise.
    */
