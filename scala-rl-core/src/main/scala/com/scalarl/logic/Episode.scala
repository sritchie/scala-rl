...
object Episode {
  ...
  /** Wrapper around a combination of state and policy. A moment in time. this wraps up a common thing that we
    * interact with...
    */
  case class Moment[Obs, A, R, M[_]](
    ...
    /** Play a single round of a game. Returns M of:
      *
      * \- pair of (the new policy that's learned, the new state you end up in) \- triple of (state you came
      * from, action you took, reward you received).
      */
    def play(implicit M: Monad[M]): M[(Moment[Obs, A, R, M], SARS[Obs, A, R, M])] =
      policy.choose(state).flatMap(act)
  }

  /** Takes a policy and a starting state and returns an M containing the final policy, final state and the
  ...
  /** Takes an initial set of policies and astate... we could definitely adapt this to do some serious
    * learning on the policies, and use the MonoidAggregator stuff.
    */
