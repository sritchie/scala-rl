...
object MonteCarlo {
  ...
  object Tracker {
    ...
    /** Returns a Tracker instance where ShouldUpdateState signals YES for every single state.
      */
  ...
  def sarsa[Obs, A, R, M[_]: Monad, T](
    ...
    Util.iterateUntilM(moment, tracker) { case m @ Moment(policy, state) =>
      m.play
    }(_.state.isTerminal)
  ...
  def processTrajectorySimple[Obs, A, R, G, M[_]](
    ...
    // of zero value for the final state, even if we use a new aggregation type.
    trajectory
      .foldLeft((valueFn, agg.monoid.zero)) { case ((vf, g), (SARS(s, a, r, s2), shouldUpdate)) =>
  ...
  def weighted[Obs, A, R, G, M[_]](
    ...
    Aggregator
      .appendMonoid[SARS[Obs, A, R, M], (G, Weight)] { case ((g, w), sars) =>
  ...
  ): (State[Obs, A, R, M], A, R) => Weight = { case (s, a, _) =>
