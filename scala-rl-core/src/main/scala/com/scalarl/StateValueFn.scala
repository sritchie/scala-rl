...
/** Along with [[ActionValueFn]], this is the main trait in tabular reinforcement learning for
  * tracking the value of a state as evidenced by the observation it returns.
  *
...
trait StateValueFn[Obs, T] { self =>
  ...
  /** Absorb a new value for the supplied observation. The behavior of this function is
  ...
  /** Transforms this [[StateValueFn]] into a new instance that applies the supplied `prepare` to
    * all incoming values before they're learned, and presents tracked T instances using the
    * `present` fn before returning them via [[stateValue]].
    *
    * @tparam the
  ...
  /** Returns a [[StateValueFn]] instance that uses the supplied semigroup T to merge values into
    * this current [[StateValueFn]].
    *
    * @param T
...
object StateValueFn {
  ...
  /** Returns an empty [[StateValueFn]] backed by an immutable map. The supplied default value will
    * be returned by [[StateValueFn.stateValue]] for any obs that's not been seen by the
    * [[StateValueFn]].
    */
  def empty[Obs, T](default: T): StateValueFn[Obs, T] =
    Base(Map.empty, default)

  /** Returns an empty [[StateValueFn]] backed by an immutable map that uses the zero of the
  ...
  /** Returns an empty [[StateValueFn]] backed by an immutable map that uses the supplied `default`
    * as a default value, and merges new learned values into the value in the underlying map using
    * the Semigroup's `plus` function.
    */
  def mergeable[Obs, T](default: T)(implicit
      T: Semigroup[T]
  ): StateValueFn[Obs, T] =
  ...
  /** Basic implementation of a [[StateValueFn]] that stores any value supplied to [[update]] in an
  ...
  /** [[StateValueFn]] implementation that implements a fold.
    ...
    * the base [[StateValueFn]]. Any value retrieved by [[stateValue]] will be passed to `present`
  ...
  /** [[StateValueFn]] implementation that merges values passed to [[update]] into the value stored
    * by the base [[StateValueFn]] using the supplied Semigroup's `plus` function.
    */
