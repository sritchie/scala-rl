/** Okay, let's see what we can do for this bandit thing. We want something that can run as a bandit, and then
  ...
  *
  * Then... do we go to the state monad, on TOP of the generator, to return the reward? Instead of returning
  * the pair directly?
  *
  * Let's give it a try!
  */
...
/** A world should probably have a generator of states and actions... and then you can use that to get to the
  * next thing. The state here is going to be useful in the Markov model; for the bandit we only have a single
  * state, not that useful.
  */
trait State[Obs, A, @specialized(Int, Long, Float, Double) R, M[_]] { self =>
  ...
  /** For every action you could take, returns a generator of the next set of rewards. This is a real world,
    * or a sample model. If we want the full distribution we're going to have to build out a better interface.
    * Good enough for now.
    */
  ...
  /** Returns a list of possible actions to take from this state. To specify the terminal state, return an
    * empty set.
    */
