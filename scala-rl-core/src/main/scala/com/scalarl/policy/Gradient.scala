...
case class Gradient[Obs, A: Equiv, R: ToDouble, T: ToDouble, S[_]](
  ...
  /** Let's try out this style for a bit. This gives us a way to convert an action directly into a
    * probability, using our actionValue Map above.
    */
  ...
  override def learn(sars: SARS[Obs, A, R, S]): This = {
    ...
    val updated = state.actions.foldLeft(valueFn) { case (vfn, a) =>
