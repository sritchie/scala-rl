...
object FrequencyTracker {
  def empty[A, B](f: A => B): FrequencyTracker[A, B] = FrequencyTracker(Vector.empty, Map.empty[B, Int], f)

  def pure[A, B](a: A, f: A => B): FrequencyTracker[A, B] = empty(f) :+ a

  def monoid[A, B](f: A => B): Monoid[FrequencyTracker[A, B]] = new Monoid[FrequencyTracker[A, B]] {
    val zero: FrequencyTracker[A, B] = FrequencyTracker.empty(f)
    def plus(l: FrequencyTracker[A, B], r: FrequencyTracker[A, B]): FrequencyTracker[A, B] =
      FrequencyTracker(l.items ++ r.items, Monoid.plus[Map[B, Int]](l.frequencies, r.frequencies), l.f)
  }
}
